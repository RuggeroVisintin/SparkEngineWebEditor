...
## New feature development through ATDD (Acceptance Test Driven Development)

When presented with the ask of developing a new feature you need to:

A) Collect acceptance criterias if not attached in the ticket
B) Use hide new functionality behind a feature flag or reuse one already available
C) Map acceptance criterias into e2e tests which should only run based on feature flag status. Do not add more tests than needed to cover the scenarios.

Acceptance criterias can be found as part of the attached ticket, if they are not available make sure to map them there.

The feature is ready to be released only when all e2e tests for that feature are green.
